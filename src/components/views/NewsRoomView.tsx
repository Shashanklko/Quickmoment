...
// Shimmer Skeleton for Hero Breaking Article
const HeroSkeleton: React.FC = () => (
  <div className="p-5 sm:p-8 rounded-3xl bg-slate-900 border border-slate-800 animate-pulse relative overflow-hidden shadow-xl">
    <div className="flex items-center justify-between mb-4">
      <div className="h-5 w-28 bg-slate-800 rounded-full" />
      <div className="h-4 w-20 bg-slate-800 rounded-md" />
    </div>
    <div className="h-7 w-4/5 bg-slate-800 rounded-lg mb-3" />
    <div className="h-4 w-full bg-slate-800/80 rounded-md mb-2" />
    <div className="h-4 w-2/3 bg-slate-800/80 rounded-md mb-6" />
...
// Shimmer Skeleton for Grid Articles
const GridCardSkeleton: React.FC = () => (
  <div className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 animate-pulse flex flex-col justify-between shadow-xs">
    <div>
      ...
      <div className="h-3.5 w-full bg-slate-100 dark:bg-slate-800/60 rounded mb-1.5" />
      <div className="h-3.5 w-4/5 bg-slate-100 dark:bg-slate-800/60 rounded mb-4" />
    </div>
...
export const NewsRoomView: React.FC<NewsRoomViewProps> = ({ onNavigateToCalculator }) => {
  ...
  const [isLoadingRss, setIsLoadingRss] = useState<boolean>(true);
  ...
  const loadCategoryNews = useCallback(async (cat: NewsCategory, force: boolean = false) => {
    ...
    try {
      const liveItems = await fetchLiveRssByCategory(cat, force);
  ...
  // Sequential, staggered loading on category switch
  useEffect(() => {
    let isSubscribed = true;

    const executeFeedSync = async () => {
      if (selectedCategory !== 'all') {
        if (!rssCache[selectedCategory]) {
          setIsLoadingRss(true);
        }
        await loadCategoryNews(selectedCategory);
      } else {
        // Load primary category first
        if (!rssCache['technology']) setIsLoadingRss(true);
        await loadCategoryNews('technology');

        // Stagger next categories to avoid API rate limits
        if (!isSubscribed) return;
        await new Promise((r) => setTimeout(r, 350));
        if (!isSubscribed) return;
        await loadCategoryNews('stockMarket');

        if (!isSubscribed) return;
        await new Promise((r) => setTimeout(r, 350));
        if (!isSubscribed) return;
        await loadCategoryNews('economy');
      }
    };
  ...
  }, [selectedCategory, loadCategoryNews, rssCache]);
    ...
    <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-16 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-start">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          {/* Live Syncing Status Banner */}
          {isLoadingRss && (
            <div className="flex items-center justify-between px-4 py-2.5 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-medium animate-pulse">
              <div className="flex items-center gap-2">
                <RefreshCw className="w-3.5 h-3.5 animate-spin text-blue-500" />
                <span>Syncing live daily intelligence from Economic Times, Livemint & Markets...</span>
              </div>
              <span className="text-[10px] opacity-75 font-mono hidden sm:inline">Real-time Feed</span>
            </div>
          )}

          {/* Skeleton Loader during initial fetch */}
          {isLoadingRss && filteredArticles.length === 0 ? (
